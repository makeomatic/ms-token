  ...
  describe('#Redis', () => {
    ...
    describe('#create & #info', () => {
      ...
      it('allows to recreate token, and erases old associated token before that', async () => {
        ...
        newuid = datum.uid
        newsecret = datum.secret

        await assert.rejects(manager.info({ uid: uid }))
        await assert.rejects(manager.info({ token: secret, encrypt: true }))
        await manager.info({ uid: newuid })

        const info = await manager.info({ token: newsecret, encrypt: true })

        assert.notEqual(uid, info.uid)
        assert.equal(newuid, info.uid)
      ...
      it('generates custom secret, type number', async () => {
        ...
        assert.ok(result.secret && /^[0-9]{6}$/.test(result.secret))
      })
